    ...
    try {
        const result = await pool.query(`
            SELECT r.*, 
                   ...
                   ) as points_of_interest
            FROM routes r
            LEFT JOIN users u ON r.created_by = u.id
            LEFT JOIN route_points rp ON r.id = rp.route_id
            LEFT JOIN points_of_interest p ON rp.point_id = p.id
            GROUP BY r.id, u.username
            ORDER BY r.created_at DESC
        `);
        
        // Filter out null points of interest and format the response
        const routes = result.rows.map(route => ({
            ...route,
            points_of_interest: route.points_of_interest[0] === null ? [] : route.points_of_interest
        }));
